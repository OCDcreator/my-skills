...
echo
printf '确认要更新外部技能并推送吗？请输入 Y 后回车继续：'
read -r CONFIRM
if [ "$CONFIRM" != "Y" ]; then
  STATUS="CANCELLED"
...
for source in "${SOURCES[@]}"; do
  ...
  echo "[$source_index/${#SOURCES[@]}] $prefix"
...
if git diff --cached --quiet; then
  if [ "$SOURCE_ERRORS" -eq 0 ]; then
    STATUS="NO_CHANGES"
    DETAIL="外部技能没有新变化，所以无需提交和推送。"
  else
    STATUS="PARTIAL"
    DETAIL="部分技能源下载失败，而且没有检测到可提交的新变化。"
  fi
  finish
  exit 0
fi
...
COMMIT_MSG="sync external skills $COMMIT_TS"
