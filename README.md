...
| [obsidian-plugin-i18n](custom/obsidian-plugin-i18n/) | Use when the user wants to translate/localize an Obsidian plugin into Chinese (or another language) — extract all user-facing strings from a plugin's main.js, translate them, optionally pop up a native GUI review window for human to approve/edit translations, then either rewrite main.js in place... |
...
| [scan-pdf-to-print-html](custom/scan-pdf-to-print-html/) | Use when a completed canonical `source-transcript.md` or other clean markdown must become faithful A4 HTML/PDF. Also use after `rewrite-doc2x-markdown` has finished OCR cleanup for scanned PDFs, textbooks, formulas, tables, diagrams, or question pages. Do not use as the default PDF-to-Markdown or... |
