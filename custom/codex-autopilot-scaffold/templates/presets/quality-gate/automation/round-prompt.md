...
Mission:
- Continue the quality-gate recovery program one active lane at a time.
...
2. Read the current `[NEXT]` lane item and restate the failing or noisy gate, target scope, and stop condition in your plan.
3. Start from the lane roadmap, lane-map entrypoints, and validation baseline before broad searching.
