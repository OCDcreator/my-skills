...
def scaffold_repo(args: argparse.Namespace) -> int:
    ...
    if args.print_next_steps:
        ...
        print("  # 2) Resolve startup intent before launch.")
        print("  #    choose exactly one startup intent before launch: preview-only / single real round / keep-running.")
        print("  #    smoke is an intermediate checkpoint for keep-running.")
        print("  #    do not report success for keep-running until health passes on the intended state file.")
        ...
        print("  python automation/autopilot.py start --profile windows --dry-run --single-round")
        print("  # Smoke is an intermediate checkpoint, not a terminal success state, when the operator intent is keep-running.")
        ...
        print("[scaffold] mac wrapper preview path:")
        ...
        print("  # Use remote Mac-side health as the real proof for a remote keep-running target.")
