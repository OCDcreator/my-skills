...
- Rendered question/example blocks must satisfy the browser-DOM contract, not just source CSS strings: `例/例题/练习` labels inside `.phycat-blockquote` render as visible `.lead-tag-example` badges, blockquotes retain the left accent rule, question option tables inside blockquotes are neutral (transparent and borderless), option table `th` and `td` share font/background/border state, and option images are large enough to read. Run `py -3 scripts/validate_rendered_handout_contract.py --html handout.html` after every build, and fix any failure before screenshot/PDF review; for final KaTeX-rendered math jobs, add `--require-katex --disallow-mathjax`. <!-- evolved 2026-06-20 -->
...
- vendored `phycat`-style example blockquotes; the left accent rule is a rendered-output hard contract, so a generic `blockquote` CSS rule must never flatten `.transcript-flow .phycat-blockquote` into a plain shaded box <!-- evolved 2026-06-20 -->
...
- `scripts/validate_rendered_handout_contract.py` — browser-DOM rendered-output gate for blockquote lead badges and left rule, neutral question option tables, readable choice images, overflow, contentless sheets, and optional final KaTeX/MathJax checks via `--require-katex --disallow-mathjax` <!-- evolved 2026-06-20 -->
