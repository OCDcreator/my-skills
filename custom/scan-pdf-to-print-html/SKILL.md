...
3. **Run the postprocess step — it is mandatory, not optional.** It activates three capabilities that the builder alone does NOT apply, and without it examples lose their quote styling and SVG covers never appear:
   `py -3 scripts/postprocess_handout_for_contract.py --html handout.html`
   - wraps every `例/例题/练习` paragraph into a `.phycat-blockquote` (without this step, example blocks render as plain paragraphs with no quote styling)
   - injects `concept-map.svg` / `concept-map.png` / `concept-map-preview.png` (if present in the job dir next to `handout.html`) as the first A4 cover sheet — this is the ONLY mechanism that turns a root-level SVG into the homepage cover, and it keys on these exact filenames
   - enforces lecture/chapter pagination (`第N讲/章/节/篇/单元/部分` and chapter-shaped `##` headings start a fresh sheet) and replaces MathJax with KaTeX
   This step is idempotent (`injectConceptCover` guards on `coverInjected`, `inject_cover_metadata` guards on `not in html`), so re-running it on an already-postprocessed HTML is safe. Skipping it silently produces examples without quote blocks and SVG covers that never appear — the single most common cause of "used the skill but examples have no quote / cover SVG is missing" defects. <!-- evolved 2026-06-23 -->
...
- `scripts/postprocess_handout_for_contract.py` — **mandatory post-build step (Markdown-Source Mode step 3)**: in-place rewrite of the built `handout.html` that (a) wraps `例/例题/练习` paragraphs into `.phycat-blockquote`, (b) injects `concept-map.svg`/`concept-map.png`/`concept-map-preview.png` as the first A4 cover sheet, (c) enforces lecture/chapter pagination, (d) replaces MathJax with KaTeX. Idempotent. Running the builder WITHOUT this step is the root cause of missing example quote blocks and missing SVG covers. <!-- evolved 2026-06-23 -->
...
**Coverage (3 evals, declared in `evals/evals.json`, schema-compatible with skill-creator):**
1. `example-blockquote-coverage` — pre-build gate flags bare-paragraph examples; post-build leaves zero bare example `<p>` outside `.phycat-blockquote`; 解析 stays out of blockquotes.
...
3. `chapter-h2-pagination` — each `第N章/节` h2 except the first starts its own sheet; the preceding sheet is marked `data-ends-before-lecture`.
