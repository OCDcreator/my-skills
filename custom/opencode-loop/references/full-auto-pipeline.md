...
If you intentionally write a repo-local `gate-review.sh` instead of using `hooks install-review`, it must read the review context generated by opencode-loop before falling back to raw Git status:

```bash
DIFF_PATCH="$(cat "$OPENCODE_LOOP_REVIEW_DIFF_FILE" 2>/dev/null || true)"
DIFF_NAMES="$(cat "$OPENCODE_LOOP_REVIEW_CHANGED_FILES_FILE" 2>/dev/null || true)"
DIFF_STAT="$(cat "$OPENCODE_LOOP_REVIEW_STAT_FILE" 2>/dev/null || true)"
```

Do not reject merely because `git diff` is empty. In execute isolation, a successful task can already be committed with a clean worktree while still having a valid `OPENCODE_LOOP_REVIEW_DIFF_RANGE`.
