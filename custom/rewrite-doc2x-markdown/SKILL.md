...
- **Question callout title line** (`lint_question_callout_title_attached`): the `> [!question]` title line must hold only the `例题N`/`例N`/`练习N` label and its source tag — never the stem body (已知…/设…/若…/求…). OCR frequently glues the stem's first sentence onto the title line; this lint anchors on the universal label, strips any source tag, and flags the callout if stem text remains on the title line. Like the analysis-paragraph lint, it is **non-auto-fixable** (deciding where to break is a semantic judgment) — the fix is to split the stem off the title line by hand or re-run Step 2.7.

**Two lint families — diagnose them separately** (added 2026-06-28 stress-test lesson). When the validator fails, sort the FAIL lines into one of two families before deciding how to fix, otherwise you can loop forever on a defect that isn't a rewrite failure:
- **Rewrite-structure lints** = real Step 2.7 rewrite-failure signals: `lint_question_callout_title_attached` (stem glued to title), `lint_choice_options` (options not in table), `lint_bare_question_starts` (example label not in callout), `lint_qa_ordering` (analysis not directly after its question), `lint_markdown_analysis_paragraphs` (analysis paragraph >300 chars). These mean the question-block STRUCTURE was not cleaned — re-run Step 2.7.
- **Formula-normalization lints** = a separate concern, NOT a rewrite failure: `lint_fraction_nesting` (`\dfrac`-in-sqrt → `\tfrac`), `lint_inline_math_spacing`, `lint_html_math`. The Step 2.7 rewrite prompt says "preserve every LaTeX construct verbatim", so a model that keeps `$\sqrt{1+\dfrac{1}{4}}$` intact is CORRECT — it must not be sent back to "re-run the rewrite" over a formula lint it was never told about. Fix these with `--fix` or a targeted formula edit, not by re-running Step 2.7.
