...
- **Rewrite-structure lints** = Step 2.7 rewrite-failure signals (`lint_question_callout_title_attached`, `lint_choice_options`, `lint_bare_question_starts`, `lint_qa_ordering`, `lint_markdown_analysis_paragraphs`, `lint_paragraph_separator` [any two prose lines — body, callout, or analysis — joined by a single `\n` merge into one paragraph in Obsidian/CommonMark; a blank line / blank `>` line is required], `lint_block_separator` [a pipe table / `<figure>` / `<table>` / `<div>` block opener glued to preceding PROSE by a single `\n` does not render as a new block — same blank-line requirement, for block elements; display-math `$$` is exempt]). Fix by re-running Step 2.7.
