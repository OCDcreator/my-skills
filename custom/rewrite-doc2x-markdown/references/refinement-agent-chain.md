...
**Full split/keep rules + examples**: see `.opencode/agents/math-comma-splitter.md` (canonical spec). **Read it in full before acting** — it covers three fusion shapes: (a) multiple labeled coordinate points crammed in one span (`$A(0,0), B(3,-2), C(5,1)$` — each point is independent, split; the comma *inside* `(x,y)` stays), (b) multiple independent equations fused by a comma (`$|AC|=\sqrt{26}, |BD|=\sqrt{26}$`), (c) multiple independent variables (`$A, B, C$`, `$k, l_1$`). <!-- evolved 2026-07-02 — `lint_list_inside_math` now DETECTS all three shapes via depth-0-comma tracking (was interval/percent-only until 2026-07-02, SILENT on these three). The depth-tracking correctly exempts structural commas inside `(x,y)`/`[a,b)`/`f(x,y)`/`{...}`. Residual hand-scan need: the lint flags any depth-0 comma between two `=`-containing units, but cannot judge whether two comma-joined equations are truly *independent* vs. a legit compound statement (rare) — spot-check flagged spans for that edge case. -->

**Does NOT do**: structure, typos, color. Only formula delimiter hygiene.

**Self-check**: `--only "lint_formula_dangling_tail,lint_list_inside_math,lint_long_inline_formula,lint_inline_math_spacing"`. `$` count must stay balanced before/after. **Ceiling note**: `lint_list_inside_math` now catches the three fusion shapes (a/b/c) via depth-0-comma detection (evolved 2026-07-02); before that it was interval/percent-only and a role-③ pass that split nothing could return a clean self-check. The residual blind spot is semantic: a flagged depth-0 comma between two equations is almost always a real split, but a compound statement (rare, e.g. a single definition `$a=1, b=2$` meant as one definition) may be flagged — use judgment.
