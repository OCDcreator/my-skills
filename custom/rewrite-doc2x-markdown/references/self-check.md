...
- [ ] **Long formulas**: display formulas > 60 chars use `\begin{aligned}` with line breaks. **Multi-equality chain inline spans** (a `$...$` over ~90 chars with ≥2 `=`) must be moved to a `$$...$$` block folded with `\begin{aligned}` — a single long expression (one `=` or none) is a legit inline span. ✅ Coarse-signal automated by `lint_long_inline_formula` (Step 4 validator).
