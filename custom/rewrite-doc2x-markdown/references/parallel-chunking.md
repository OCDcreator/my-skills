...
- **Dispatch runtime** (host-native subagent by default; or the pre-registered OpenCode agents `md-cleaner` → `analysis-retypesetter` → `question-block-rewriter` if detected): see `references/opencode-agent-invocation.md` for the detection command, invocation template, and the **mandatory caller-side verification contract** (never trust the agent's self-report — diff against a pre-run backup and re-run the validator independently).
