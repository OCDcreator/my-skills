...
- <!-- added 2026-06-29 --> **A long multi-equality chain belongs in a `$$...$$` display block folded with `\begin{aligned}`.** An inline `$a = b = c = ...$` over ~90 chars with ≥2 equalities is hard to read; use `$$\begin{aligned} a &= ... \\ &= ... \end{aligned}$$`. A single long expression (one `=` or none, e.g. a big `\dfrac{1}{N}\sum...`) is a legitimate inline span and does NOT need this. Coarse-signal lint `lint_long_inline_formula` points at the chain; judge in context.
